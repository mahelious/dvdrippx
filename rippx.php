...
try {
    ...
    if (count($files_for_processing) < 1) {
    ...
    foreach ($files_for_processing as $file_to_process) {
        ...
        $filename_parts = explode(DS, $file_to_process);
        ...
        $processing_file = $settings['RIPX_PROCESSING_DIR'] . DS . date('YmdHi') . '_' . $filename;
        ...
        $output_filename = implode(DS, [
            $settings['RIPX_RIP_OUTPUT_DIR'],
            $dirname,
            strstr($filename, '.mkv', true) . '.mp4'
        ]);
        
        $encode_cmd = sprintf('HandBrakeCLI -Z "%s" -i "%s" -o "%s"', PRESET_GENERAL_HQ_1080, $processing_file, $output_filename);
...
function get_mkv_files($mkv_dir, $depth = 0)
{
    $mkv_files = [];
    ...
    foreach ($dir_contents as $filename) {
        ...
        if (is_dir($file)) {
            $mkv_files = array_merge($mkv_files, get_mkv_files($file, $depth+1));
        } else 
